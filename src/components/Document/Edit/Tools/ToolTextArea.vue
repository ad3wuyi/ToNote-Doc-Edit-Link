...
<template>
  <template v-if="tool.append_print == null">
    <Vue3DraggableResizable :key="tool.id" :initH="Number(tool.tool_height)" :initW="Number(tool.tool_width)" :minW="70"
      ...
      :data-can-drag-tool="tool.can_drag_tool" :data-user="tool.user_id" :data-doc="tool.document_upload_id"
      :data-id="tool.id" :data-print-id="tool?.append_print?.id" class-name-active="active-class"
      ...
      <div class="text-wrapper">
        <input type="text" v-model="tool.value" class="textareaTool w-100 h-100" :data-doc="tool.document_upload_id"
          :data-id="tool.id" :data-print-id="tool?.append_print?.id" :data-name="tool.tool_name"
